...
|Diffusion Policy: Visuomotor Policy Learning via Action Diffusion|arxiv2024|在视觉-运动控制任务中，文章基于强化学习的框架使用扩散模型生成robot动作序列，逐步优化生成的动作轨迹以获得连续、平滑的动作轨迹，对于策略优化部分采用了逆强化学习（Inverse Reinforcement Learning）的思想；图像特征提取就是传统的CNN-Based和Transformer-Based；|Diffusion结合Action，这个方法确实是获得连续且平滑动作序列的不错方式|
