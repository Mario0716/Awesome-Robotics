...
|Instant Policy: In-Context Imitation Learning via Graph Diffusion|ICLR2025||先说这个模型的训练过程，可以简单的理解为GNN+Diffusion Policy，对于得到每个节点的特征部分（不同的节点作用是不同的，分别代表着动作，机器人状态->基本就是夹爪的开关状态，目标物的信息），都要先融合上一个节点的特征，然后再进行K（加噪声）+K（去噪声）步的扩散生成|
